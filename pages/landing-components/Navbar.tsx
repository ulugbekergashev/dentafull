...
export default function Navbar({ onOpenDemoModal, scrollToSection }: NavbarProps) {
  ...
  const navTextClass = isScrolled ? "text-slate-600 hover:text-blue-600" : "text-slate-700 hover:text-blue-600";
        ...
        <div className="flex items-center justify-between gap-6">
          ...
          <div className="hidden lg:flex items-center gap-4 shrink-0">
            ...
            <button
              ...
              className="px-5 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm transition-all shadow-md shadow-blue-500/15 active:scale-95 cursor-pointer whitespace-nowrap"
            >
              Tizimga Kirish →
            </button>
          </div>
        ...
        <div className="lg:hidden bg-white border-b border-slate-200 px-4 pt-2 pb-6 space-y-1 shadow-lg">
          {NAV_LINKS.map(link => (
            <button
              ...
              className="block w-full text-left px-3 py-2.5 rounded-xl text-sm font-semibold text-slate-700 hover:bg-slate-50 hover:text-blue-600 transition-colors"
            >
              {link.label}
            </button>
          ))}
          <div className="pt-4 border-t border-slate-100 space-y-2">
            ...
            <button
              onClick={() => window.location.href = '/login'}
              className="w-full text-center py-3 rounded-xl bg-blue-600 hover:bg-blue-700 text-white font-semibold text-sm transition-all cursor-pointer"
            >
              Tizimga Kirish →
            </button>
          </div>
